...
colnames = names(chain1) |> x -> x[3:(end-1)] .|> Symbol
a = mapreduce(
    n -> Array(
...
##
include("posterior_plots.jl")

gen_infs = generated_quantities(model, chn[1:5:end, :, [1, 2, 3, 4, 6]])
...
for idx = 1:8
    ...
    _onsets = (onsets[idx, :]+onsets_hcw[idx, :])[:]
    _reported = (reported[idx, :]+reported_hcw[idx, :])[:]
